...
const Product = (props) => {
  ...
  alertonClick = () =alert("Item Has Been added to cart please refresh")
  useEffect(() => {
    let prev_items = JSON.parse(localStorage.getItem("cart")) || [];
    //addItems(prev_items);
    //const total = () => {
    setCartTotal(prev_items.reduce((acc, item) => acc + item.price, 0));
    //};

    //const totalQuantity = () => {
    setQuantity(prev_items.reduce((acc, item) => acc + item.quantity, 0));
    //};
